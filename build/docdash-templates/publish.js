...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    /**
     * config(templates.default.staticFiles.include) 字段功能扩展
     * 1. 支持导入文件夹
     * {
     *     "src": "./img/disp",
     *     "out": "/img"
     ...
     */
    (() => {
        let static_file_paths = conf.default.staticFiles.include || conf.default.staticFiles.paths || [];
        let static_file_filter = new (require('jsdoc/src/filter')).Filter(conf.default.staticFiles);
        let static_file_scanner = new (require('jsdoc/src/scanner')).Scanner();
        
        static_file_paths.forEach(function(filePath) {
            if(typeof filePath == "string") {
                ...
                filePath = {
                    ...
                    out: path.resolve(path.join(outdir, filePath.out || ""))
                }
            }
            if(fs.statSync(filePath.src).isDirectory()) {
                let extraStaticFiles = static_file_scanner.scan([filePath.src], 10, static_file_filter);
                extraStaticFiles.forEach(function(fileName) {
                    fs.cp(fileName, path.join(filePath.out, path.basename(fileName)), function(err) { err && console.error(err)})
                });
            } else {
                fs.cp(filePath.src, filePath.out, function(err) { err && console.error(err)})
            }
    ...
    //         filePath = typeof filePath == "string" ? {src: filePath, out: outdir} : filePath
    //         filePath.out = path.join(outdir, filePath.out)
    //         console.log(outdir, filePath.out)

    //         var extraStaticFiles = staticFileScanner.scan([filePath.src], 10, staticFileFilter);

    //         extraStaticFiles.forEach(function(fileName) {
    //             var sourcePath = fs.toDir(filePath.src);
    //             // var toDir = fs.toDir( fileName.replace(sourcePath, outdir) );
    //             var toDir = fs.toDir( fileName.replace(sourcePath, filePath.out) );
    //             fs.mkPath(toDir);
    //             copyFile(fileName, path.join(toDir, path.basename(fileName)), function(err){if(err) console.err(err);});
    //         });
    //     });
